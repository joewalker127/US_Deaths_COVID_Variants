...
#
#
# Modified by Jo Walker to generate state-level variant proportion estimates
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
#dat <- readRDS(file = '~/Documents/GitHub/SARS-CoV-2_Genomic_Surveillance/Code/example_data.RDS')
dat <- read.csv("~/Documents/GitHub/SARS-CoV-2_Genomic_Surveillance/Code/example_data.csv")
...
# all the AY sublineages ("Delta plus")
...
dat[dat$VARIANT %in% B351,"VARIANT"] <- "B.1.351" # beta
...
dat[dat$VARIANT %in% B621,"VARIANT"] <- 'B.1.621' # mu

B427 = sort(unique(dat$VARIANT)[grep("B\\.1\\.427",unique(dat$VARIANT))])
B427 = B427[which(B427 %notin% voc)] #vector of the B427s to aggregate
dat[dat$VARIANT %in% B427,"VARIANT"] <- "B.1.427" # epsilon
...
dat[dat$VARIANT %in% B429,"VARIANT"] <- "B.1.427" # epsilon
...
dat[dat$VARIANT %in% B529,"VARIANT"] <- "B.1.1.529" #omicron

B1640 = sort(unique(dat$VARIANT)[grep("(B\\.1\\.640)",unique(dat$VARIANT))])
B1640 = B1640[which(B1640 %notin% voc)] #vector of the B1640s to aggregate
dat[dat$VARIANT %in% B1640,"VARIANT"] <- "B.1.640"

C36 = sort(unique(dat$VARIANT)[grep("C.36",unique(dat$VARIANT))])
C36 = C36[which(C36 %notin% voc)] #vector of the C36s to aggregate
dat[dat$VARIANT %in% C36,"VARIANT"] <- "C.36"
...
for(i in unique(all.wkly2$USA_or_HHSRegion))
  {
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "Other") %>% select(WEEK_END,Share) %>% plot(type="l", main = i, ylim = c(0, 1.1), las = 1, xlab = "", ylab = "Variant Share")
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "B.1.1.7") %>% select(WEEK_END,Share) %>% lines(type="l", col = "red")
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "C.37") %>% select(WEEK_END,Share) %>% lines(type="l", col = "blue")
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "P.1") %>% select(WEEK_END,Share) %>% lines(type="l", col = "orange")
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "B.1.617.2") %>% select(WEEK_END,Share) %>% lines(type="l", col = "purple")
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "B.1.621") %>% select(WEEK_END,Share) %>% lines(type="l", col = "pink")
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "B.1.1.529") %>% select(WEEK_END,Share) %>% lines(type="l", col = "green")
  filter(all.wkly2, USA_or_HHSRegion==i, Variant == "B.1.427") %>% select(WEEK_END,Share) %>% lines(type="l", col = "brown")
  }

# save the results to file
write.csv(
  x = all.wkly2,
  file = "~/Documents/GitHub/SARS-CoV-2_Genomic_Surveillance/variant_share_weekly_weighted.csv",
  row.names = FALSE
)

## Weighted variant proportions: each state ------------------------------------------------------
...
all.wkly = expand.grid(Variant = voc,
                       Week_of = wks,
                       USA_or_HHSRegion = unique(dat2$STUSAB))[, 3:1]
...
## generate sequence counts by lineage, location and date
# raw counts of each variant in each week and each region
raw_counts_REG <- aggregate(count ~ VARIANT2 + WEEK_END + STUSAB,
                            data = dat2,
                            FUN  = sum,
                            drop = FALSE)

# convert region names to character
raw_counts_REG$STUSAB <- as.character(raw_counts_REG$STUSAB)
...
write.csv(
  x = all.wkly2,
  file = "~/Documents/GitHub/SARS-CoV-2_Genomic_Surveillance/state_variant_share_weekly_weighted.csv",
  row.names = FALSE
)
